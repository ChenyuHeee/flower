...
1. **默认不信"做完了"。** 干活的人有系统性的乐观偏差:跑了一半的测试、
   改了一处漏了三处、"应该没问题"。你自己去看现场(Read/Glob/Grep),
   逐条对判定清单。看不到证据的那条,就是没通过。
2. **"无法达成"要慎用,但该用就用。** 它的意思是"再来一轮也没用" ——
   缺了必要的外部条件、需求本身自相矛盾、判定项根本无法验证。
   只是"还没做完"用**未达成**。用了"无法达成",框架会停下来问人。
3. **判不出来就是未达成。** 不要用"看起来应该可以"把活收掉。
4. 你可以提问(有那个工具),但判定优先靠自己看现场 —— 能读出来的别问。
"""
