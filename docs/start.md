...
**并行数量没有上限。** 那把写锁是**限时抢**的(0.25 秒):抢不到就直接写。
理由是阻塞版会传染 —— 一个 flower 卡在写终端(终端不读了、被 Ctrl+S 挂起、
正在崩)就会攥着锁不放,同终端上**所有**别的 flower 跟着冻住。
交错只是难看,冻住是真的没法用。实测八个进程并行、25 万字节输出,零撕裂。

`manifest.json` 也是并行安全的:每次落盘重读一遍、按 `run` 归并,
所以同一个目录里并行跑也不会互相冲掉账。

> 但**一个目录一个 flower** 仍然是建议:它们会抢同一份 `lineage.json`,
