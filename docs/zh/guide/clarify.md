...
有一类错误不是这样:**目标理解错了**。它是唯一一类**清理上下文会让它变严重**的错误。现场被丢掉之后,
...
### 触发时机:三处接线,一个新字段都没加 {#触发时机三处接线一个新字段都没加}
...
### 确认书落在哪:必须是挂上去的那个工作台 {#确认书落在哪必须是挂上去的那个工作台}
