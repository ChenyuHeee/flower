...
### `is_overflow`:把硬错变成当场换代 {#is_overflow把硬错变成当场换代}
...
### 一颗地雷:写交接那一轮必须豁免阈值 {#一颗地雷写交接那一轮必须豁免阈值}
